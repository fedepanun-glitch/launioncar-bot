...
console.log("=== BOT v3.1 - PARSER ROBUSTO + FIND EXACTO ===");
...
app.post("/webhook",async function(req,res){
  ...
  try {
    ...
    if (!parsed) {
      // Si el modelo respondió con ✅ pero el JSON no parsea, NO mentir al usuario
      if (/✅|Vencimiento OK|Cobro OK|Venta OK|registrad/i.test(texto)) {
        console.error("[PARSER FAIL] Modelo dijo OK pero JSON no parsea. Texto:", texto.substring(0,200));
        parsed = {accion:"responder",datos:{},mensaje:"❌ Hubo un problema procesando tu mensaje. Mandalo de nuevo más simple, por favor (un dato por vez)."};
      } else {
        // Es una respuesta de chat normal (no una acción)
