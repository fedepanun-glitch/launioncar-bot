...
var historiales = {};
var SYSTEM = "Sos el asistente de La Union Car SRL. Interpretas mensajes en argentino informal. CAMIONES: UC-01=Enrique, UC-02=Juan Benitez, UC-03=Fernando, UC-04=Gustavo, UC-05=Pablo, UC-06=Juan Romero. PRODUCTOS: gasoil=gas_oil_g2, premium=gas_oil_premium, super=nafta_super, infinia=infinia_diesel. ACCIONES: registrar_compra, registrar_venta, registrar_cobro, registrar_gasto, registrar_venc_camion, registrar_venc_chofer, consultar_stock, consultar_saldo, consultar_ventas_hoy, consultar_alertas, responder. TIPOS VENC CAMION: vtv, seguro, habilitacion_cnrt, extintor, cisterna_adr, service. TIPOS VENC CHOFER: registro_conducir, seguro_art, cargas_peligrosas_cnrt, psicofisico, conduccion_defensiva, libreta_sanitaria. Responde siempre JSON puro sin markdown: {\"accion\":\"nombre\",\"datos\":{\"litros\":0,\"precio_litro\":0,\"producto\":\"\",\"cliente\":\"\",\"proveedor\":\"\",\"camion\":\"\",\"monto\":0,\"tipo\":\"\",\"fecha_vencimiento\":\"\",\"descripcion\":\"\"},\"mensaje\":\"\"}";
...
async function run(accion,datos) {
  try {
    ...
    if (accion==="registrar_venc_chofer") { var ch=await find("choferes","apellido",datos.chofer); if (!ch) ch=await find("choferes","nombre",datos.chofer); if (!ch) return {ok:false,msg:"No encontre al chofer "+datos.chofer}; if (!datos.fecha_vencimiento) return {ok:false,msg:"Falta la fecha"}; var e=await db.from("documentos_choferes").insert([{chofer_id:ch.id,tipo:datos.tipo||"registro_conducir",fecha_vencimiento:datos.fecha_vencimiento,notas:datos.descripcion||null}]); if (e.error) return {ok:false,msg:e.error.message}; return {ok:true,msg:"Vencimiento OK\n"+ch.apellido+" - "+(datos.tipo||"registro_conducir")+"\n"+datos.fecha_vencimiento}; }
